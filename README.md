...
| Pause | **Escape**, or the MENU button. Resume, save to one of three slots, copy or download the turn log, change the sound or the theme, or quit to the title. A save may be taken **mid-turn** — a walking token and an open buy prompt both survive a reload |
...
it is what the 520 unit tests pin down.
...
That is what lets 520 unit tests run in ~8 s with no jsdom, and it is the seam a
...
```bash
npm test                # 520 unit tests, plain Node, ~8 s
